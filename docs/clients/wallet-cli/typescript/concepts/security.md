...
The CLI reads no secret from argv and from no dedicated environment variable — those would leak through shell history, `ps` output, and CI logs. A shell variable may be the *source* of what you pipe in, but do not leave it `export`ed. Secrets enter only via:
