...
- **`tx status` says `pending` for a long time** — the node has seen the transaction but has not mined it; keep polling. If it never leaves `pending`/`not_found` past your deadline, that is **unknown state**, not failure: `not_found` says only that the node you asked does not know it, and it may still be in another node's mempool. Reconcile via a SolidityNode or a block explorer and confirm it did not land before considering a resend — resending blind is how double payments happen.
- **`tx status` says `confirmed` — is it irreversible?** — No. `confirmed` reads the FullNode's unconfirmed view and means only "mined, with a receipt". Query a SolidityNode when you need finality.
