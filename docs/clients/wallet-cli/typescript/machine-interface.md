...
The CLI reads no secret from argv and from no dedicated environment variable — those would leak through shell history, `ps` output, and CI logs. A shell variable may be the *source* of what you pipe in, but do not leave it `export`ed. Secrets enter through two channels only:
...
3. Or poll yourself with `tx status`, which has a **four-state model**:
   ...
   |---|---|---|
   | `confirmed` | Mined with an execution receipt (`blockNumber` present) | yes |
   | `failed` | Included and reverted / rejected | yes |
   | `pending` | Seen but not yet mined | no — keep polling |
   | `not_found` | Unknown to the queried node | no — past your deadline this is **unknown**, not failed |

   > **`confirmed` means mined, not irreversible.** This command and `--wait` both read the
   > FullNode's *unconfirmed* view: `confirmed` says the transaction is in a block and its receipt
   > is settled, **not** that the block is solidified and can no longer be reverted. Query a
   > SolidityNode when you need finality.

   > **A `not_found` timeout is not a failure.** It says only that *the node you asked* does not
   > know the transaction; it may still be in another node's mempool. Never resend on that basis —
   > that is how double payments happen. Past your deadline, treat it as **unknown state** and
   > reconcile via a SolidityNode or a block explorer before deciding.
...
```bash
# Capture the output and the exit code before parsing: a pipeline returns jq's status, and on a
# failure envelope `.data.txId` evaluates to null with jq still exiting 0 — the wallet-cli failure
# would be swallowed.
...
deadline=$(( $(date +%s) + 120 ))       # your own deadline

while :; do
  # The query needs the same treatment: as a pipeline you would read jq's exit code, and on an
  # error envelope `.data.state` evaluates to null — which polls forever as an "empty" state.
  status_out=$(wallet-cli tx status --txid "$txid" --network tron:nile -o json)
  status_code=$?
  if [ "$status_code" -ne 0 ]; then
    printf '%s\n' "$status_out" | jq -r '.error.code' >&2
    exit "$status_code"                      # the query failed; do not silently continue
  fi
  state=$(printf '%s' "$status_out" | jq -er '.data.state') || exit 1

  case "$state" in
    confirmed) echo "mined: $txid"; break ;;
    failed)    echo "execution failed: $txid" >&2; exit 1 ;;   # must abort, never loop
    *)         # pending / not_found
      if [ "$(date +%s)" -ge "$deadline" ]; then
        echo "unknown state, reconcile by hand: $txid" >&2; exit 2
      fi
      sleep 3 ;;
  esac
done
