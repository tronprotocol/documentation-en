...
|---|---|---|---|
| `params[0]` | string | yes | Non-negative block height (`0x`-prefixed hex or decimal) or tag: `latest` / `earliest` / `finalized` (`pending` and `safe` are explicitly unsupported, see Error responses) |
