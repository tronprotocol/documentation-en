...
|---|---|---|---|
| `params[0]` | string | yes | Contract address: 20-byte hex or 21-byte Tron hex beginning with `41`, with or without `0x` (base58check is not accepted) |
| `params[1]` | string | yes | Storage slot index, hex-encoded with or without `0x`; values shorter than 32 bytes are left-padded with zeros |
...
| `params[2]` is a valid hex or decimal number | `-32602` | `QUANTITY not supported, just support TAG as latest` |
| `params[2]` is neither a valid tag nor a valid non-negative hex/decimal number | `-32602` | `invalid block number` |
