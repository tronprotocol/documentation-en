...
| `params[1]` | string \| object | yes | Block tag: a tag string, or a `{"blockNumber": "0x..."}` / `{"blockHash": "0x..."}` object (EIP-1898). **Only `latest` is supported** (other tags raise an error; specific heights, even if they exist, are rejected) |
...
| `params[1]` tag is `earliest` / `pending` / `finalized` | `-32602` | `TAG [earliest \| pending \| finalized] not supported` |
