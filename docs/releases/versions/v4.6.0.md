...
The delegate relationship storage optimization is a network parameter of TRON. It is disabled by default and can be enabled by initiating a proposal.
...
The memo fee is a network parameter of TRON. After GreatVoyage-v4.6.0 (Socrates) is deployed, the default value is ‘0’, and the unit is ‘sun’. It can be enabled by specifying a non-zero value by initiating a proposal, for example, ‘1000000’, indicating that the transaction with memo will require an additional 1 TRX fee.
